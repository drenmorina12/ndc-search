<div class="w-full max-w-4xl mx-auto space-y-6 relative">
    ...
    @if (session()->has('message'))
        ...
        <div class="block sm:hidden bg-amber-50 dark:bg-amber-900/50 border border-amber-200 dark:border-amber-800 rounded-md p-3">
            <div class="flex items-center gap-2 text-amber-800 dark:text-amber-200 text-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Desktop: Floating message -->
        <div class="hidden sm:flex absolute left-1/2 transform -translate-x-1/2 z-20 items-center gap-2 text-amber-800 dark:text-amber-200 text-xs font-medium bg-amber-50 dark:bg-amber-900/50 px-2.5 py-1.5 rounded shadow-lg border border-amber-200 dark:border-amber-800 backdrop-blur-sm whitespace-nowrap" style="top: -40px;">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-amber-600 dark:text-amber-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
            {{ session('message') }}
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
        <form wire:submit.prevent="search" class="flex flex-col sm:flex-row gap-4">
            <input type="text" wire:model.defer="ndcInput"
    ...
    <div wire:loading wire:target="search"
        class="absolute left-1/2 transform -translate-x-1/2 z-20 flex items-center justify-center gap-3 text-gray-900 dark:text-white text-sm font-medium bg-white dark:bg-zinc-800 px-4 py-2 rounded-md shadow-lg border border-blue-200 dark:border-blue-900"
        style="top: 85px;">
                            ...
                            @foreach ($results as $result)
                                <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                                    <td class="px-6 py-4 text-gray-900 dark:text-white font-medium">
                                        {{ $result['drug']['ndc_code'] ?? $result['ndc_code'] }}</td>
                                    <td class="px-6 py-4 text-gray-700 dark:text-gray-300">
                                        {{ $result['drug']['brand_name'] ?? '-' }}</td>
                                    <td class="px-6 py-4 text-gray-700 dark:text-gray-300">
                                        {{ $result['drug']['labeler_name'] ?? '-' }}</td>
                                    <td class="px-6 py-4 text-gray-700 dark:text-gray-300">
                                        {{ $result['drug']['product_type'] ?? '-' }}</td>
    ...
    @if (empty($results) && $ndcInput)
        <div
            class="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-8 text-center">
